...
public class InventoryProvider extends ContentProvider {
    ...
    private Uri insertInventory(Uri uri, ContentValues values) {
        ...
        // Check that the re-order quantity is valid
        Integer reOrderQuantity = values.getAsInteger(InventoryEntry.COLUMN_ITEM_REORDER_QUANTITY);
        if (reOrderQuantity == null || !InventoryEntry.isValidReOrderQuantity(reOrderQuantity)) {
            throw new IllegalArgumentException("Inventory requires valid re-order quantity");
        }

        // Check that the supplier email is not null
        String supplier = values.getAsString(InventoryEntry.COLUMN_SUPPLIER_EMAIL);
        if (supplier == null) {
            throw new IllegalArgumentException("Item requires a Supplier's Email");
        }
